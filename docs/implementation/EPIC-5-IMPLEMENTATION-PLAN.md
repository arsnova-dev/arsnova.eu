...
| **Frontend preset-toast / quiz-edit** | Toggles für enableSoundEffects, enableRewardEffects, enableMotivationMessages, enableEmojiReactions, backgroundMusic (Dropdown). |
| **Frontend session-vote** | Platzhalter: `showRewardEffect`, `motivationMessage` (pickRandom aus MESSAGES_*). Keine echte Scorecard, kein Streak-UI, keine Sounds. |
