...
**Implementierungsstand (2026-03-12):** Phase 1–5 und Teile von Phase 6 umgesetzt. **Phase 2:** App, Home, Top-Toolbar, Join, Preset-Toast, Session-Host (und weitere Bereiche) mit i18n/$localize markiert; **~212 Messages** extrahiert; `messages.en.xlf` mit EN-Targets für alle Einheiten. **Phase 4:** Sprachwähler → Locale-URL (Top-Toolbar, localStorage, Vollreload); Locale-Helfer in `core/locale-from-path.ts`. **Phase 5:** Legal-Seiten laden Inhalte pro Locale (`imprint.{locale}.md`, `privacy.{locale}.md`) mit Fallback auf `de` bei 404; Hinweis bei Sprachwechsel auf Quiz Edit/New: `LocaleSwitchGuardService` (Quiz-Edit/Quiz-New registrieren ungespeicherte Änderungen), Toolbar zeigt Bestätigungsdialog „Sprache wechseln? Ungespeicherte Änderungen gehen verloren.“ mit „Abbrechen“ / „Trotzdem wechseln“. **Phase 6:** `i18nMissingTranslation: "warning"` in `angular.json` (Build-Option); für strikte CI auf `"error"` umstellbar.

**Nächste Schritte (optional):** Weitere Locales (fr, it, es) in angular.json + Übersetzungsdateien; Phase 6.1–6.3 (visuelle Prüfung Mobile-First, DoD 6.2, Stichproben Datum/Einheiten); restliche Phase-2-Bereiche (Session-Vote, Quiz-List/Edit/Preview, Help, Feedback) vollständig markieren.
