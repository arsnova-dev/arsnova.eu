...
| **Admin**   | Admin/Betreiber         | `/admin` (ggf. `/admin/sessions`) |
...
*(Umsetzung: Story 2.1a um Host-Token bei `session.create` erweitern; neue Backend-Prozedur oder Middleware zur Host-Token-Prüfung; Frontend: Host-Route-Guard bzw. Check in Host/Present-Komponenten.)*
